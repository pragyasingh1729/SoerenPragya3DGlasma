...
int Eigenvalues(COMPLEX A[3][3], DOUBLE w[3]){
  ...
  sqrt_p = sqrt(std::abs(p));

  phi = DOUBLE(27.0) * ( DOUBLE(0.25)*SQR(c1)*(p - c1) + c0*(q + DOUBLE(27.0)/DOUBLE(4.0)*c0));
  phi = (DOUBLE(1.0)/DOUBLE(3.0)) * atan2(sqrt(std::abs(phi)), q);
  
  c = sqrt_p*cos(phi);
  s = (DOUBLE(1.0)/M_SQRT3)*sqrt_p*sin(phi);
...
int Eigensystem(COMPLEX A[3][3], COMPLEX Q[3][3], DOUBLE w[3]){
  ...
  MaxAbsEval=std::max(std::abs(w[0]),std::abs(w[1]));
  MaxAbsEval=std::max(MaxAbsEval,std::abs(w[2]));
