...
\
SU_Nc_ALGEBRA_FORMAT DeviationBuffer[SUNcAlgebra::NumberOfGenerators];
...
for(int alpha=0;alpha<SUNcGroup::MatrixSize;alpha++){\
    w[alpha]=SQR(Lattice::aScale/UGaugeTransformed->a[0])*(U0G[alpha]+U0DGxM[alpha])+SQR(Lattice::aScale/UGaugeTransformed->a[1])*(U1G[alpha]+U1DGyM[alpha])+(UGaugeTransformed->N[2]>1)*SQR(Lattice::aScale/UGaugeTransformed->a[2])*(U2G[alpha]+U2DGzM[alpha]);\
}\
\
...
#define MONITOR_GAUGE_DEVIATION() \
    ...
    for(int a=0;a<SUNcAlgebra::NumberOfGenerators;a++){\
        MaxDeviation=std::max(MaxDeviation,std::abs(DeviationBuffer[a])); \
