...
#include "TMDNUCLEUS/NuclearData.cpp"
#include "TMDNUCLEUS/CorrelationFunction.cpp"
#include "TMDNUCLEUS/GBWSampling.cpp"
#include "TMDNUCLEUS/TMDbased.cpp"
...
namespace Simulation {
	...
	void CreateInfoFile(){
		...
		OutStream << "MEff aka Infrared Cutoff= " << Parameters::MEff << " "<<"(in units of QsAperp)"<<std::endl;
		OutStream << "SigmaLOverAz= " << Parameters::SigmaLOverAz << std::endl;
		OutStream << "Center of Mass energy= " << Parameters::CoMEnergy*0.197<<" "<<"GeV"<< std::endl;
		OutStream << "Lorentz Factor= " << Parameters::CoMEnergy*0.197/2.0 << std::endl;
		OutStream << "aTrans in Fm= " << Parameters::aXInFm <<std::endl;
		OutStream << "aLong in Fm= " << Parameters::aZInFm <<std::endl;
		OutStream << "Lattice in Fm= "<< Parameters::aXInFm*Lattice::N[0] << " x "<< Parameters::aYInFm*Lattice::N[1] << " x "<< Parameters::aZInFm*Lattice::N[2]<<std::endl;
		OutStream << std::endl;
	...
	void Run(INT MPI_RNG_SEED){
		...
		ChargeDistributions::ChargeFluctuation(SUNcChargeDensity::CFL,'L');

		SUNcChargeDensity::CFL->SynchronizeGhostCells();

	    TmdBasedCollision::SetWilsonLines(SUNcChargeDensity::rhoL,SUNcChargeDensity::CFL,SUNcWilsonLines::VLeft,'L');

		SUNcChargeDensity::rhoL->SynchronizeGhostCells();

		//SET RIGHT SITTING NUCLEUS //
		ChargeDistributions::ChargeFluctuation(SUNcChargeDensity::CFR,'R');
		
		SUNcChargeDensity::CFR->SynchronizeGhostCells();
		
		TmdBasedCollision::SetWilsonLines(SUNcChargeDensity::rhoR,SUNcChargeDensity::CFR,SUNcWilsonLines::VRight,'R');
		
		SUNcChargeDensity::rhoR->SynchronizeGhostCells();
		...
		//FOR LEFT SITTING NUCLEUS//
		InitialConditions::SingleSheet::SingleSheet(SUNcChargeDensity::rhoL,0,(Lattice::N[2]-2)/2,SUNcChargeDensity::RhoLT1,SUNcWilsonLines::VLeft,'L',SUNcChargeDensity::JpStat,SUNcChargeDensity::Jp);
		
		//FOR RIGHT SITTING NUCLEUS//
		InitialConditions::SingleSheet::SingleSheet(SUNcChargeDensity::rhoR,Lattice::N[2]/2,Lattice::N[2]-1,SUNcChargeDensity::RhoRT1,SUNcWilsonLines::VRight,'R',SUNcChargeDensity::JmStat,SUNcChargeDensity::Jm);
