namespace GaussLawRestoration2D{
    
    //GAUSS VIOLATION INDEX -- MACRO TO RETURN THE POSITION OF A CERTAIN COMPONENT OF COLORED FIELD
#define GaussViolationIndex(x,y,z,a) ((a)+SUNcAlgebra::NumberOfGenerators*(SUNcElectricFields::E->Index3D((x),(y),(z))))
    ...
    #pragma omp for reduction( + : SqrSumViolation)  reduction ( max : MaxViolation)
	   ...
	   // SYNCHROIZE BOUNDARIES FOR GHOST CELL //
	   std::memcpy(&GaussViolation[GaussViolationIndex(0,0,-1,0)]          , &GaussViolation[GaussViolationIndex(0,0,0,0)]	          ,SUNcAlgebra::NumberOfGenerators*E->N[0]*E->N[1]*sizeof(SU_Nc_ALGEBRA_FORMAT));
	   std::memcpy(&GaussViolation[GaussViolationIndex(0,0,E->N[2],0)]     , &GaussViolation[GaussViolationIndex(0,0,E->N[2]-1,0)]    ,SUNcAlgebra::NumberOfGenerators*E->N[0]*E->N[1]*sizeof(SU_Nc_ALGEBRA_FORMAT));
	   
	   //std::cerr << "###SYNCHRONIZE BOUNDARIES FRO GAUSS LAW VIOLATIONS" << std::endl;
    }
    ...
    void Restore(GaugeLinks *U,ElectricFields *E, ChargeDensity *Jp,ChargeDensity *Jm){
	   ...
	   // ALLOCATE MEMORY //
	   GaussViolation=(SU_Nc_ALGEBRA_FORMAT *) _mm_malloc(SUNcAlgebra::NumberOfGenerators*(E->N[0])*(E->N[1])*(E->N[2]+2)*sizeof(SU_Nc_ALGEBRA_FORMAT),64);
